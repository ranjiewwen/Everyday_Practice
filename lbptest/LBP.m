...
% bsizey*bsizex  %计算块的大小 3*3
bsizey=ceil(max(maxy,0))-floor(min(miny,0))+1;
bsizex=ceil(max(maxx,0))-floor(min(minx,0))+1;

% Coordinates of origin (0,0) in the block %边界2
...
% Calculate dx and dy; %228-3=225
dx = xsize - bsizex;
dy = ysize - bsizey;

% Fill the center pixel matrix C. %中心区域
C = image(origy:origy+dy,origx:origx+dx);
d_C = double(C);
bins = 2^neighbors;

% Initialize the result matrix with zeros. %返回中心区域的LBP编码
...
if isstruct(mapping)
    ...
    for i = 1:size(result,1)
        for j = 1:size(result,2)
            result(i,j) = mapping.table(result(i,j)+1); %+1因为table从0开始
        end
    end
end
